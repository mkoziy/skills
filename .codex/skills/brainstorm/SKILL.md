...
```bash
bash ~/.codex/skills/brainstorm/scripts/resolve-rules.sh brainstorm-rules.md ~/.codex
```
...
- **show rules**: run `bash ~/.codex/skills/brainstorm/scripts/resolve-rules.sh brainstorm-rules.md ~/.codex` and display the output. If the output is empty, tell the user no custom rules are configured at either level. Otherwise, to determine the source, check if `.claude/brainstorm-rules.md` exists and is non-empty (project-level) — if not, the output came from user-level (`~/.codex/brainstorm-rules.md`). Tell the user which level it came from.
- **add/update project rules**: write content to `.claude/brainstorm-rules.md` in the current working directory.
- **add/update user rules**: write content to `~/.codex/brainstorm-rules.md`.
- **clear project rules**: delete `.claude/brainstorm-rules.md`.
- **clear user rules**: delete `~/.codex/brainstorm-rules.md`.

Project-level rules (`.claude/brainstorm-rules.md`) take precedence over user-level rules (`~/.codex/brainstorm-rules.md`). When both non-empty files exist, only project-level rules are loaded. Empty files are treated as absent and fall through to the next level. See `~/.codex/skills/brainstorm/references/custom-rules.md` for full documentation on the rules mechanism.

**CRITICAL: this skill must NEVER modify its own files (skills, scripts, references). The ONLY files it may create or modify for rules management are `.claude/brainstorm-rules.md` and `~/.codex/brainstorm-rules.md`. If the user asks to change the skill's behavior, suggest creating a plan — do not edit skill files directly.**
