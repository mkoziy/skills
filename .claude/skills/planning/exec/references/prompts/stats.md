...
- Use the meta file's mtime as the spawn timestamp.
- Read the LAST event in the corresponding `.jsonl` for finish time and final `usage` block.
- The usage block contains `input_tokens`, `cache_creation_input_tokens`, `cache_read_input_tokens`, `output_tokens`. Sum them per-subagent.
