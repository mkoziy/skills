...
After reading a prompt file, replace ALL placeholders with actual values before passing to a subagent. Subagents run in fresh contexts and cannot resolve relative paths themselves.

Always substitute: `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, `DEFAULT_BRANCH`, `SKILL_ROOT` (this skill's own absolute directory path, so subagents can locate its `scripts/`, `references/`, and `prompts/`), `RESOLVE_SCRIPT` (absolute path to `scripts/resolve-file.sh` under `SKILL_ROOT`), `USER_RULES` (resolved custom rules content from the rules loading step, or empty string if no rules found), and phase-specific values (`FINDINGS_LIST`, `REVIEW_PHASE`, `DIFF_COMMAND`).
...
1. **Read review.md as a playbook (NOT as a subagent prompt)** — resolve `prompts/review.md` through the override chain and read it from this main session. It tells YOU (the orchestrator) which specialist agents to fan out for the current `REVIEW_PHASE`. Substitute `DEFAULT_BRANCH`, `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, `SKILL_ROOT`, and `REVIEW_PHASE` in the resolved content. Then follow the playbook FROM THIS SESSION: launch the specified Agent tool calls in a single message for parallel execution. Subagents do not have Agent tool access, so the fanout MUST be initiated from the main orchestrator.
