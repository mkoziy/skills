...
Use this for the finalize agent after all reviews pass (replace `DEFAULT_BRANCH`, `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, and `${SKILL_ROOT}`):
...
STEP 4 - LOG PROGRESS:
Log results: bash ${SKILL_ROOT}/scripts/append-progress.sh PROGRESS_FILE_PATH "finalize: completed"
Then pipe details: echo "- rebase: <success/failed>
- commits before: N, after: M
- squashed: <list of squashed commits, or none>
- validation: <passed/failed>" | bash ${SKILL_ROOT}/scripts/append-progress.sh PROGRESS_FILE_PATH
