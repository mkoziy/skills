...
Custom rules let you inject project-specific or personal conventions into the planning workflow (make, exec, and make's auto-review subagent). Rules are free-form markdown loaded at skill invocation time and applied as additional instructions alongside the skill's built-in behavior.
...
Each skill runs `scripts/resolve-rules.sh planning-rules.md` via Bash at startup. It falls back to the `$CLAUDE_PLUGIN_DATA` env var for the user-level tier if set, otherwise skips straight to project-level / empty. Outputs the first file found (project, then user) or empty output if neither exists.

## Managing Rules

Ask `make` to manage rules:
...
- **make's auto-review step**: rules become additional review criteria for convention adherence
