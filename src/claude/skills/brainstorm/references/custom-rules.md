...
2. **User-level**: `~/.claude/brainstorm-rules.md`
...
The skill runs `resolve-rules.sh brainstorm-rules.md ~/.claude` via Bash at startup. Outputs the first file found (project, then user) or empty output if neither exists.
...
- **add/update user rules** — writes to `~/.claude/brainstorm-rules.md`
- **clear project rules** — deletes `.claude/brainstorm-rules.md`
- **clear user rules** — deletes `~/.claude/brainstorm-rules.md`
