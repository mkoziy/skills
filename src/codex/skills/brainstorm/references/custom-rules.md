...
2. **User-level**: `~/.codex/brainstorm-rules.md`
...
The skill runs `resolve-rules.sh brainstorm-rules.md ~/.codex` via Bash at startup. Outputs the first file found (project, then user) or empty output if neither exists.
...
- **add/update user rules** — writes to `~/.codex/brainstorm-rules.md`
- **clear project rules** — deletes `.claude/brainstorm-rules.md`
- **clear user rules** — deletes `~/.codex/brainstorm-rules.md`
