...
This file is a playbook for the main orchestrator session — NOT a prompt to spawn into a subagent. Subagents do not have access to the Agent tool, so the parallel fanout below must be initiated from the main session.
...
Resolve each agent's prompt file using the resolve script (run these bash invocations first, then assemble the agent prompts):
...
After ALL 5 agents return, produce a STRICT bullet-list report — no prose summary, no narrative. Format requirements:

- Group findings by severity: CRITICAL, MAJOR, MINOR. Use a heading per severity (`### CRITICAL`, `### MAJOR`, `### MINOR`). Skip a severity heading if it has zero findings.
- Under each heading, one bullet per finding: `- <agent-name>: <file:line> — <description>`
- If two agents reported the same file:line + same issue, merge into one bullet: `- quality+implementation: main.go:12 — ...`
- Do NOT verify, fix, or dismiss findings here — the fixer agent does that.
- After the bullet list: `Total: <N> findings (<C> critical, <M> major, <m> minor)`.

## Critical-only mode (2 agents)

Used when `REVIEW_PHASE` is `critical`.

Resolve only `quality.txt` and `implementation.txt`. Replace `DEFAULT_BRANCH`, then prepend the READ-ONLY preamble plus:
...
In your next assistant response, emit 2 Agent tool_use blocks together.

After BOTH agents return, produce the same STRICT bullet-list report. Additional rule:
- Drop any MINOR findings. Only CRITICAL and MAJOR headings appear here.
