...
Use this for the fixer agent after collecting review findings (replace `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, `FINDINGS_LIST`, and `${CLAUDE_PLUGIN_ROOT}` before passing):
...
- Commit fixes: bash ${CLAUDE_PLUGIN_ROOT}/scripts/stage-and-commit.sh "fix: address code review findings" <changed-files>

STEP 5 - LOG PROGRESS (after commit):
Log details: echo "- confirmed: <list>
- false positives: <list>
- fixes: <what changed>
- validation: <what passed>" | bash ${CLAUDE_PLUGIN_ROOT}/scripts/append-progress.sh PROGRESS_FILE_PATH
